...
#define  TAMSTR  	50
#define  TAMCARTAO	16
#define  MAX_ELEMENTOS	50								//maximo de elementos na estrutura de fila
#define true 1
#define false 0
...
#define  ARQUIVO_FILA				"FILA.DAT"			//arquivo da fila
#define  NOME_ESTABELECIMENTO       "DRIVE THRU DO BURGER KONG"
...
//STRUCT DA FILA
typedef struct{
	registro_pagamento reg[ MAX_ELEMENTOS ];
	int inicio;
	int qtdElementos;
}FILA;
...
int 	QtdElementos(FILA *fila);
void 	exibirFila(FILA *fila);
bool 	inserirElementoFila(FILA *fila, registro_pagamento reg);
bool 	excluirElementoFila(FILA *fila, registro_pagamento *reg);
void 	printQtdFila(FILA *fila);
void	LerArquivoPagamentos (FILA *fila);
...
void  falha_abrir_arquivo_cartoes     (void)
{
	printf ("\nFALHA AO ABRIR [%s]!", ARQUIVO_DE_CARTOES);
	getch();
	exit(0);
}
void falha_abrir_arquivo_fila (void)
{
	system ("cls");
	printf ("FALHA AO ABRIR [%s]!", ARQUIVO_FILA);
	getch();
	exit(0);
}
...
void cadastra_produtos_default (void)
{
   ...
   {  {1, "X-Burger"          , 7.0},
   	  {2, "Batata Frita"      , 5.0},
   	  {3, "Refrigerante Lata" , 5.0},
   	  {4, "Suco de Laranja"   , 4.0},
   	  {5, "Sorvete"           , 4.0} };
...
//instrucoes da fila
void iniciarFila(FILA *fila){ 			//tambem serve para reiniciar a fila
	fila->inicio = 0;
	fila->qtdElementos = 0;
}
int QtdElementos(FILA *fila){ 			//retorna a quantidade de elementos na fila
	return fila->qtdElementos;
}
void exibirFila(FILA *fila){			//exibe os elementos na fila
	int i = fila->inicio;
	int temp;
	
	for(temp = 0; temp< fila->qtdElementos; temp++){
		printf("\nCodigo: %d -- Valor %.2f",  fila->reg[i].Codpgto, fila->reg[i].Valorpgto);
		i = (i+1) % MAX_ELEMENTOS ;
	}
}
bool inserirElementoFila(FILA *fila, registro_pagamento reg){	//insere novo elemento na fila, sendo esses elementos registros de pagamentos
	if (QtdElementos(fila)>= MAX_ELEMENTOS)
		return false;
	int posicao = (fila->inicio + fila->qtdElementos) % MAX_ELEMENTOS;
	fila->reg[posicao] = reg;
	fila->qtdElementos++;
	return true;
}
bool excluirElementoFila(FILA *fila, registro_pagamento *reg){
	if(fila->qtdElementos == 0)
		return false;
	*reg = fila->reg[fila->inicio];
	fila->inicio = (fila->inicio+1)%MAX_ELEMENTOS;
	fila->qtdElementos--;
	return true;
}
void printQtdFila(FILA *fila){
	printf("\nA fila possui %d elementos", QtdElementos(fila));
}
void LerArquivoPagamentos (FILA *fila){
	FILE *A;
   registro_pagamento Rler; 
   A = fopen (ARQUIVO_FILA, "r");
   if (A==NULL)
      falha_abrir_arquivo_fila();  
   else
   {    
   	    while ( !feof(A) )
   	    { 
...
void gerarFila(registro_pagamento Rpgto){
	FILE *A;
	A = fopen(ARQUIVO_FILA, "a");
	if (A == NULL) 
		falha_abrir_arquivo_fila();
	fwrite (&Rpgto, sizeof(Rpgto), 1, A );
	fclose(A);
...
void salvarFila(FILA *fila){           //salva fila apos retirar elemento
    FILE *A;
    int i = fila->inicio;
    registro_pagamento reg;
    int temp;
    A = fopen(ARQUIVO_FILA, "w");
    for(temp = 0; temp< fila->qtdElementos; temp++){
        reg = fila->reg[i];
